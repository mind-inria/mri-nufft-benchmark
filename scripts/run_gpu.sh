#!/usr/bin/env bash
# Reproduce only the GPU part of the benchmark: assets -> all scenarios x GPU
# backends x actions (benchmark + memory suites, host and GPU-resident input)
# -> report.html. See run_all.sh for the full (CPU+GPU) sweep and how-to.md
# for details.
#
...
# torchkbnufft-gpu builds one FFT plan per coil with no batching, so its
...
# it still runs on every other scenario. See run_all.sh's SLOW_BACKENDS.
GPU_BACKENDS_FAST=cufinufft,gpunufft
...
# $GPU_BACKENDS ("all") or $GPU_BACKENDS_FAST ("fast") for that row.
SCENARIOS=(
...
for scenario in "${SCENARIOS[@]}"; do
  ...
  [ "${backend_tier}" = "fast" ] && backends="${GPU_BACKENDS_FAST}"
